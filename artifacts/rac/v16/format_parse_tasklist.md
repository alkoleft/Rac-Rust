...
**Agent**
- [~] `rac agent admin list`: захват с `auth=pwd,os`, `os-user` и `descr` непустыми; сопоставить `unknown_tag/unknown_flags/unknown_tail` с полями `auth/os-user/descr` и проверить несколько записей. — Статус: захват [x], анализ [~], описание [~]
- [x] `rac agent version`: получить захват запроса/ответа и подтвердить порядок/формат ответа. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac agent admin register`: получить захват запроса/ответа, восстановить порядок полей. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac agent admin remove`: получить захват запроса/ответа, восстановить порядок полей. — Статус: захват [x], анализ [x], описание [x]
...
- [x] `rac cluster admin register`: захват с `--auth=pwd,os` и `--auth=os`; подтвердить `auth_flags` и смысл `unknown_0` в хвосте запроса. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac cluster admin remove`: получить захват запроса/ответа, восстановить порядок полей. — Статус: захват [x], анализ [x], описание [x]
...
**Connection**
- [x] `rac connection list`: захват с ненулевым `blocked-by-ls` для подтверждения поля `blocked_by_ls`. — Статус: захват [~], анализ [x], описание [x]
- [x] `rac connection list`: захват с `--process` для определения порядка полей запроса. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac connection list`: захват с `--infobase` для определения порядка полей запроса. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac connection list`: захват с `--infobase-user` для определения порядка полей запроса. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac connection list`: захват с `--infobase-pwd` для определения порядка полей запроса. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac connection info`: захват запроса/ответа для определения порядка полей запроса. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac connection disconnect`: получить захват запроса/ответа, восстановить порядок полей. — Статус: захват [x], анализ [x], описание [x]

**Infobase**
- [x] `rac infobase summary list`: захват с разными значениями, чтобы идентифицировать `u8 tag` в записи. — Статус: захват [~], анализ [x], описание [x]
- [x] `rac infobase info`: получить v11-захват payload и сопоставить неизвестные строки/байты и `tail[28]` с полями. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac infobase summary info`: получить захват и подтвердить формат ответа. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac infobase summary update`: получить захват запроса/ответа, восстановить порядок полей. — Статус: захват [x], анализ [x], описание [x]
...
- [x] `rac lock list --session`: захват, чтобы проверить наличие дополнительных полей в запросе. — Статус: захват [x], анализ [x], описание [x]
...
- [x] `rac process info --licenses`: определить `gap_license_0` и подтвердить хвостовые поля `use`/`reserve`/`memory-excess-time`. — Статус: захват [~], анализ [x], описание [x]
...
**Session**
- [x] `rac session info`: получить v11-захват payload для подтверждения порядка/наличия полей. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac session list --licenses`: захват запроса, чтобы зафиксировать наличие и порядок `licenses` флага. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac session info --licenses`: захват запроса, чтобы зафиксировать наличие и порядок `licenses` флага. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac session terminate`: получить захват запроса/ответа, восстановить порядок полей. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac session interrupt-current-server-call`: получить захват запроса/ответа, восстановить порядок полей. — Статус: захват [x], анализ [x], описание [x]
...
**Rule**
- [x] `rac rule apply --partial`: захват для подтверждения enum `apply_mode`. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac rule list`: захват с непустыми `infobase-name`, `application-ext` и `priority>0`; проверить `object-type` и `rule-type`. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac rule info`: захват с `rule-type=always/never` и `priority>0` для подтверждения enum и порядка. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac rule insert`: захват с непустыми `infobase-name`, `application-ext`, `priority>0` для подтверждения смещений. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac rule update`: захват с непустыми полями для подтверждения смещений и наличия предварительного `rule info`. — Статус: захват [x], анализ [x], описание [x]
- [x] `rac rule remove`: захват ошибки (неуспешный запрос) для описания формата ошибок. — Статус: захват [x], анализ [x], описание [x]
