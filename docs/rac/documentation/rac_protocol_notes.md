...
```bash
scripts/rac/capture_rac_command.sh <name> <rac args...>
```

Example:

```bash
scripts/rac/capture_rac_command.sh cluster_list cluster list
scripts/rac/capture_rac_command.sh cluster_info cluster info --cluster 1619820a-d36f-4d8a-a716-1516b1dea077
```
...
- `logs/session_<...>/...`
- `artifacts/rac/<label>.hex` (extracted response payload examples)

Example extraction:

```bash
scripts/rac/extract_rac_response_example.sh logs/session_1771098941_320377_127_0_0_1_42586 0x42 session_list_response
```

## Next reference

Method map extracted from multiple commands is in `docs/rac/documentation/rac_method_map.md`.
