...
3. Decode streams:
   - `cargo run -p rac_protocol --quiet --bin rac_decode -- <session>/client_to_server.stream.bin`
   - `cargo run -p rac_protocol --quiet --bin rac_decode -- <session>/server_to_client.stream.bin`
...
1. Present confirmed facts first (bytes, method IDs, frame format).
...
6. Do not use absolute offsets in the final report; describe layouts as a sequential field order.
7. Represent unknown bytes as `skip(N)` within the field sequence.
8. Heuristics are allowed during analysis, but the final decoding algorithm must not rely on heuristics.

## Final Format Requirements

- Do not use absolute offsets in the final report. Always describe layouts as a sequential field order.
- If bytes are unknown, represent them as `skip(N)` within the field sequence.
- Heuristics are allowed during analysis, but the final decoding algorithm must not rely on heuristics.
- Prefer a “message map” on the intermediate step: `field -> skip -> field -> skip`.

## Session Analysis Notes

- All fields are laid out sequentially; model the record as a stream, not fixed offsets.
- If offsets drift between captures, it is likely a variable-length field (usually `str8`).
- When ambiguous `u32` values appear near unknown 4-byte blocks, re-check for `u64` fields.
- At the start of analysis, list all fields visible in `rac` output; track missing fields and use them to justify unknown slots.
- If multiple fields show identical values (often zero), ask for load generation or changed data to disambiguate.
- Always save both the capture and the `rac` output for the same session.
